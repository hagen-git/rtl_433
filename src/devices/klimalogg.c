...
play with the -l option (5000-15000 range), the decoder should be disabled
because of the poor user experience. Fixing this require a large rewrite.
...
static int klimalogg_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    printf("crc_calc=0x%02x crc_bs=0x%02x\n",crc, msg[10]);
    /* Extract parameters */
